...
def process(params):
    ...
    try:
        ...
        if os.path.isdir(tmp_dir):
            ...
            print("rm tree", tmp_dir)
    return results_dict


def test_rouge(cand, ref, lang='en', temp_dir='./tmp'):
    ...
    if isinstance(cand, str):
        ...
        references = ref
    if lang == 'zh':
        candidates, word2idx = chinese2id(candidates, {})
        references, _ = chinese2id(references, word2idx)
...
def chinese2id(texts, word2id, lang='zh'):
    nts = []
    for text in texts:
        nt = []
        if lang == 'zh':
